...
st.write(
    ...
    "people and safety equipment using YOLO11."
)
...
def load_model():
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            ...
            "Please upload the trained YOLO11 best.pt file."
        )
...
try:
    ...
    st.success("✅ YOLO11 model loaded successfully")

except Exception as e:

    st.error("❌ Could not load the YOLO11 model.")

    st.code(str(e))

    st.info(
        "Make sure the trained YOLO11 'best.pt' file "
...
def analyze_results(result):
    ...
    no_hardhat = detected_classes.count(
        NO_HARDHAT
    )

    no_vest = detected_classes.count(
        NO_VEST
    )
...
if uploaded_file is not None:
    ...
    if file_name.endswith(
        ...
        st.image(
            annotated_rgb,
            caption="VisionGuard YOLO11 Detection",
...
st.caption(
    "VisionGuard AI | YOLO11 Computer Vision PPE Safety Monitoring"
)
